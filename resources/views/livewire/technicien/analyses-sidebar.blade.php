        ...
        <div class="p-3 space-y-2">
            @forelse($analysesParents as $parent)
                <div class="relative group">
                    ...
                    <button type="button"
                            ...
                            class="w-full text-left p-3 rounded-lg border transition-all duration-200
                                   ...
                                   {{ $parent['status'] === 'TERMINE' 
                                      ...
                                      : ($parent['status'] === 'EN_COURS' ? 'bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-700' : '') }}">
                    ...
                    {{-- Bouton terminer individuel (affiché uniquement si l'analyse est prête et pas encore terminée) --}}
                    @if($parent['is_ready'] && $parent['status'] !== 'TERMINE' && auth()->user()->type === 'technicien')
                        <button
                            wire:click="markAnalyseAsCompleted({{ $parent['id'] }})"
                            wire:loading.attr="disabled"
                            wire:target="markAnalyseAsCompleted"
                            class="absolute top-2 right-2 z-10 p-1.5 bg-green-600 hover:bg-green-700 dark:bg-green-700 dark:hover:bg-green-800 disabled:opacity-50 text-white rounded-md transition-colors shadow-sm"
            ...
            {{-- Bouton de finalisation globale (uniquement si toutes les conditions sont remplies) --}}
            @if(count($analysesParents) > 0 && auth()->user()->type === 'technicien')
                <div class="pt-4 border-t border-gray-200 dark:border-gray-700 mt-4">
                    @if($this->canFinalizePrescription())
                        {{-- Toutes les analyses sont terminées - Prescription déjà finalisée --}}
                    ...
                    @else
                        {{-- Pas encore prêt - Bouton désactivé avec explication --}}
