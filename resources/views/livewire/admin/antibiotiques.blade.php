<div class="p-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200">
            <div class="bg-gray-50 px-6 py-4 border-b border-gray-200 rounded-t-xl">
                <div class="flex justify-between items-center">
                    <h6 class="font-semibold text-gray-900">Liste des Antibiotiques ({{ $antibiotiques->total() }})</h6>
                    ...
                    <table class="w-full">
                        <thead class="bg-gray-50 border-b border-gray-200">
                            <tr>
                                ...
                                <th class="text-left py-4 font-medium text-gray-900">Antibiotiques</th>
                                <th class="text-left py-4 font-medium text-gray-900">Famille</th>
                                <th class="text-center py-4 font-medium text-gray-900">Bactéries</th>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach($antibiotiques as $key => $antibiotique)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-6 py-4">
                                        <span class="bg-gray-100 text-gray-800 px-2 py-1 rounded-md text-sm font-medium">{{ $key+1}}</span>
                                    </td>
                                    <td class="py-4 pr-4">
                                        <div class="font-medium text-gray-900">{{ $antibiotique->designation }}</div>
                                        @if($antibiotique->commentaire)
                                        <div class="text-sm text-gray-500 truncate max-w-xs">{{ $antibiotique->commentaire }}</div>
                                        @endif
                                    </td>
                                    <td class="py-4 pr-4">
                                        <span class="bg-blue-100 text-blue-800 px-2 py-1 rounded-md text-sm font-medium">{{ $antibiotique->famille->designation }}</span>
                                    </td>
                                    <td class="py-4 text-center">
                                        <span class="bg-purple-100 text-purple-800 px-2 py-1 rounded-md text-sm font-medium">{{ $antibiotique->bacteries_count }} bactéries</span>
                                    </td>
                                    <td class="py-4 text-center">
                                    ...
                                    <td class="py-4 text-center">
                                        <div class="flex justify-center space-x-2">
                                            ...
                                            <button wire:click="duplicate({{ $antibiotique->id }})" 
                                                    class="bg-gray-100 text-gray-700 hover:bg-gray-200 p-2 rounded-lg transition-colors"
                                                    title="Dupliquer">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <button wire:click="toggleStatus({{ $antibiotique->id }})" 
                                                    class="{{ $antibiotique->status ? 'bg-red-100 text-red-700 hover:bg-red-200' : 'bg-green-100 text-green-700 hover:bg-green-200' }} p-2 rounded-lg transition-colors"
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <h5 class="text-xl font-medium text-gray-900 mb-2">Aucun antibiotique trouvé</h5>
                    <p class="text-gray-600 mb-4">Commencez par créer votre premier antibiotique.</p>
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                        <div class="flex items-center">
                            <span class="bg-blue-100 text-blue-800 px-2 py-1 rounded-md text-sm font-medium mr-3">{{ $key+1 }}</span>
                            <div>
                                <h4 class="text-sm font-medium text-gray-900">{{ $antibiotique->designation }}</h4>
                                <p class="text-xs text-gray-500">{{ $antibiotique->famille->designation }}</p>
                            </div>
                        </div>
    ...
    @if($currentView === 'show' && $antibiotique)
        <div class="bg-white rounded-xl shadow-sm border border-gray-200">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div class="space-y-4">
                        <div class="flex justify-between py-2 border-b border-gray-100">
                            ...
                            <span class="bg-gray-100 text-gray-800 px-2 py-1 rounded-md text-sm font-medium">{{ $antibiotique->id }}</span>
                        </div>
                        <div class="flex justify-between py-2 border-b border-gray-100">
                            <span class="font-medium text-gray-600">Antibiotiques :</span>
                            <span class="font-medium text-gray-900">{{ $antibiotique->designation }}</span>
                        </div>
                        <div class="flex justify-between py-2 border-b border-gray-100">
                            <span class="font-medium text-gray-600">Famille :</span>
                            <span class="bg-blue-100 text-blue-800 px-2 py-1 rounded-md text-sm font-medium">{{ $antibiotique->famille->designation }}</span>
                        </div>
                        <div class="flex justify-between py-2 border-b border-gray-100">
                    ...
                    <div class="space-y-4">
                        <div class="flex justify-between py-2 border-b border-gray-100">
                            ...
                            <span class="text-gray-900">{{ $antibiotique->created_at ? $antibiotique->created_at->format('d/m/Y H:i') : 'N/A' }}</span>
                        </div>
                        <div class="flex justify-between py-2 border-b border-gray-100">
                            <span class="font-medium text-gray-600">Modifié le :</span>
                            <span class="text-gray-900">{{ $antibiotique->updated_at ? $antibiotique->updated_at->format('d/m/Y H:i') : 'N/A' }}</span>
                        </div>
                        @if($antibiotique->commentaire)
                        <div class="py-2">
                            <span class="font-medium text-gray-600">Commentaire :</span>
                            <p class="mt-1 text-gray-900 whitespace-pre-line">{{ $antibiotique->commentaire }}</p>
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="mt-8">
                    <h4 class="text-lg font-medium text-gray-900 mb-4">Bactéries associées ({{ $antibiotique->bacteries->count() }})</h4>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                            @foreach($antibiotique->bacteries as $bacterie)
                                <div class="flex items-center space-x-2 bg-gray-50 p-2 rounded-lg">
                                    ...
                                    <span class="text-sm text-gray-900">{{ $bacterie->designation }}</span>
                                </div>
                            @endforeach
                        </div>
                    @else
    ...
    @if(in_array($currentView, ['create', 'edit']))
        <div class="bg-white rounded-xl shadow-sm border border-gray-200">
            <div class="{{ $currentView === 'create' ? 'bg-blue-50' : 'bg-yellow-50' }} px-6 py-4 border-b border-gray-200 rounded-t-xl">
                <h6 class="font-semibold {{ $currentView === 'create' ? 'text-blue-900' : 'text-yellow-900' }} flex items-center">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="p-6">
                <form wire:submit.prevent="{{ $currentView === 'create' ? 'store' : 'update' }}">
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            <label for="famille_id" class="block text-sm font-medium text-gray-700 mb-2">Famille *</label>
                            <select 
                                wire:model="famille_id" 
                                id="famille_id" 
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('famille_id') border-red-500 focus:ring-red-500 focus:border-red-500 @enderror"
                                required
                            >
                            ...
                            @error('famille_id') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                        </div>
                        
                        <div>
                            <label for="designation" class="block text-sm font-medium text-gray-700 mb-2">Antibiotiques *</label>
                            <input 
                                type="text" 
                        ...
                        <div>
                            ...
                            <textarea 
                                ...
                                id="commentaire" 
                                rows="3" 
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('commentaire') border-red-500 focus:ring-red-500 focus:border-red-500 @enderror"
                            ></textarea>
                            @error('commentaire') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                        </div>
                    ...
                    <div class="flex space-x-4 mt-6">
                        <button 
                            ...
                            class="bg-gray-300 hover:bg-gray-400 text-gray-700 px-6 py-2 rounded-lg flex items-center transition-colors"
                        >
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
