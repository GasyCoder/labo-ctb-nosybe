    ...
    <div class="container">
        ...
        <div class="main-header">
            <div class="header-left">
                <div class="logo-section">
                    ...
                    @else
                        <div
                            style="width: 150px; height: 100px; background: #2E8B57; color: white; text-align: center; line-height: 100px; font-size: 10pt; font-weight: bold;">
            ...
            <div class="header-right">
                <div class="barcode-section" style="text-align: center;">
                    ...
                    @if ($barcodeBase64)
                        <img src="{{ $barcodeBase64 }}" alt="Code-barres"
                            style="max-width: 120px; height: 40px; display: block; margin: 0 auto;">
                    @else
                        <div
                            style="font-family: monospace; border: 1px solid #000; padding: 5px; font-size: 8pt; text-align: center;">
                            {{ $prescription->reference }}
                        </div>
                    @endif
        ...
        <div class="patient-section">
            <table class="patient-table">
                <tr>
                    ...
                    <td class="patient-value">{{ $prescription->patient->numero_dossier ?? $prescription->reference }}
                    </td>
                ...
                <tr>
                    ...
                    <td class="patient-value" colspan="4">{{ strtoupper($prescription->patient->civilite ?? '') }}
                        {{ strtoupper($prescription->patient->nom) }}
                        {{ strtoupper($prescription->patient->prenom ?? '') }}</td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td class="patient-value">{{ $prescription->prescripteur->nom ?? '' }}
                        {{ $prescription->prescripteur->prenom ?? '' }}</td>
                </tr>
                <tr>
        ...
        <table class="analyses-table">
            ...
            <tbody>
                ...
                @foreach ($prescription->prelevements as $prelevement)
                    @php
                        // On remplace les anciens champs par ceux définis dans ajouterPrelevement()
                        $prixUnitaire = $prelevement->prix ?? 0; // prix du prélèvement
                        $quantite = $prelevement->quantite ?? 1; // quantité (par défaut 1)
                        $montantPrelevement = $prixUnitaire * $quantite;
                        $totalPrelevements += $montantPrelevement;
                        $remisePrelevement = 0; // remise par défaut à 0
                    @endphp
                    <tr>
        ...
        <div class="resume-section">
            <div class="resume-left">
                ...
                <div><strong>Montant encaissé :</strong> {{ number_format($paiement ? $paiement->montant : 0, 2) }}
                    {{ $formatArgent }}</div>
                <div><strong>Solde restant :</strong>
                    {{ number_format(max(0, $total - ($paiement ? $paiement->montant : 0)), 2) }} {{ $formatArgent }}
                </div>
        ...
        <div class="footer">
            ...
            <div class="footer-line">
                @if ($nifEntreprise)
                    NIF: {{ $nifEntreprise }}
                @endif
                @if ($nifEntreprise && $statutEntreprise)
                    -
                @endif
                @if ($statutEntreprise)
            ...
            <div class="footer-line">Siège social: Antananarivo - Tél: +261 34 XX XXX XX - Email: contact@laboratoire.mg
            </div>
            <div class="footer-line">Comptes bancaires: MCB TANA: 00000 00000 00000000000 - BNI-CL TANA: 00000
                021579201023</div>
        </div>
    </div>
</body>
